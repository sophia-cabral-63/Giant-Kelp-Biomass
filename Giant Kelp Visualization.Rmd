...
#Utilized Data Entry/Table 2: Macrocystis pyrifera standing crop, plant density, and loss rates with SE. 
...
kelp_frond_density <- ggplot(averaged_data, aes(x = date, y = avg_fronds, color = Site)) + 
  ...
  scale_color_manual(
    values = c("AQUE" = "#DBDBDB", "MOHK" = "#818589"),   
...
```{r Frond Density 2022-2026, include=TRUE}
#Reed, D, R. Miller. 2025. SBC LTER: Reef: Seasonal Kelp Forest Community Dynamics: Abundance and size of Giant Kelp ver 2. Environmental Data Initiative. https://doi.org/10.6073/pasta/387497d5319cf8d539226e5b3f9b16ae. Accessed 2026-01-29.

#Utilized Data Entry/Table 6: Plant frond number and size. 

frond_number <- read.csv("plantfrond.csv")

filtered_data <- frond_number %>%
  filter(SITE %in% c("AQUE", "MOHK")) %>%
  filter(F_1M != "-99999")
...
averaged_data <- filtered_data %>%
  group_by(DATE, SITE) %>%
  summarize(avg_fronds = mean(F_1M, na.rm = TRUE), .groups = 'drop')
  

averaged_data$DATE <- as.Date(averaged_data$DATE)

frond_number <- ggplot(averaged_data, aes(x = DATE, y = avg_fronds, color = SITE)) + 
  geom_point(size = 1.75) +   
  geom_line() +
  labs(
       x = "Date",
  ...
  scale_color_manual(
    values = c("AQUE" = "#DBDBDB", "MOHK" = "#818589"),   
    labels = c("AQUE" = "Arroyo Quemado Reef", "MOHK" = "Mohawk Reef")   
  ) 


frond_number

#ggsave("frond number short.jpg", frond_number, width = 14, height = 6)
```
